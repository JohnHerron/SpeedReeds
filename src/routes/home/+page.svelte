...
<body>
    <section class="flex h-screen flex-col items-center justify-between p-6 bg-zinc-800 text-zinc-400">
        ...
        <main class="max-w-3xl break-words flex flex-col justify-center gap-8">
            ...
            <div>
                <div id="type_text" class="text-6xl font-bold text-zinc-100">{selectionText[currentIndex]}</div>
            </div>
            ...
            <textarea class="text-2xl font-bold resize-none bg-zinc-800 min-h-[75%] max-h-[75%] min-w-[700px] max-w-[700px]" readonly>{selectionText}</textarea>
            <button class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-full content-center self-center" on:click={startAutomaticChange}>Start</button>
